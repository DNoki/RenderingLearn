...
namespace Graphics
{
...
#if DEBUG
    com_ptr<ID3D12Debug3> g_D3D12Debug;
#endif // DEBUG
    ...
    void Initialize()
    {
        ...
        if (SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(g_D3D12Debug.put()))))
        {
        ...
        CHECK_HRESULT(g_Factory->MakeWindowAssociation(Application::g_Hwnd, DXGI_MWA_NO_ALT_ENTER)); // 防止 DXGI 响应 Alt+Enter
        SET_DEBUGNAME(g_Factory.get(), _T("IDXGIFactory"));
        ...
        UINT nodeCount = g_Device->GetNodeCount(); // 查询物理适配器数目
        ASSERT(nodeCount > 0);

        SET_DEBUGNAME(g_Adapter.get(), _T("IDXGIAdapter"));
        SET_DEBUGNAME(g_Device.get(), _T("Device"));
