...
void Texture2D::DirectCreate(DXGI_FORMAT format, UINT64 width, UINT height, UINT16 arraySize, UINT16 mipLevels)
{
    ...
    CHECK_HRESULT(GraphicsManager::GetInstance().GetGraphicsContext()->GetDevice()->CreateCommittedResource(
        &heapType,             // 默认堆类型
...
void Texture2D::Create(DXGI_FORMAT format, UINT64 width, UINT height, UINT16 arraySize, UINT16 mipLevels)
{
    m_ResourceDesc = CD3DX12_RESOURCE_DESC::Tex2D(format, width, height, arraySize, mipLevels);
    //m_ResourceStates = D3D12_RESOURCE_STATE_COPY_DEST; // 初始状态为拷贝目标
...
void Texture2D::DispatchCopyTextureData(const void* data)
{
    // 取得一个闲置拷贝命令列表
    auto* commandList = CommandListPool::Request<CopyCommandList>();
    commandList->Reset();

    commandList->DispatchUploadBuffer(this, data);
...
void Texture2D::InitDescriptor()
{
    ...
    m_SRV = DescriptorAllocator::Allocat(*GraphicsManager::GetInstance().GetGraphicsContext(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
