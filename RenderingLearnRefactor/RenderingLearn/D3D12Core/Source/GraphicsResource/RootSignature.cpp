...
/*
    ...
    根签名与 API
        根签名需绑定着色器的参数槽（Parameter Slot）。并且参数槽需要与着色器 Register 对应。
        Slot 能绑定以下类型
            ·Descriptor Table
                绑定描述符堆的第一个描述符句柄
                注：不要使用 Descriptor Table 来存储 CBV（不使用CreateConstantBufferView()）
            ·Root Descriptor
                注：不要将 CBV 分组到具有不同更新频率的 CBV 描述符表中。不要将 SRV、UAV 使用 根描述符绑定。
            ·Root 32bit Constant
...
void RootSignature::Finalize(D3D12_ROOT_SIGNATURE_FLAGS Flags)
{
    ...
    if (FAILED(GraphicsContext::GetCurrentInstance()->GetDevice()->CheckFeatureSupport(D3D12_FEATURE_ROOT_SIGNATURE, &rsFeatureData, sizeof(rsFeatureData))))
